...
function PlayerVideo({ 
  ...
  return (
    <div className="w-full h-full flex flex-col items-center">
      <div 
        className={`relative rounded-xl overflow-hidden ${getPlayerBorderColor()} transition-all duration-300 transform ${isPolice && !isLocal ? 'hover:scale-105' : ''} w-full h-56 md:h-64`}
        onClick={handleClick}
        role={isPolice && !isLocal ? "button" : undefined}
        aria-label={isPolice && !isLocal ? `Select player ${username || id}` : undefined}
        tabIndex={isPolice && !isLocal ? 0 : undefined}
      >
        {/* Loading state */}
        {!videoLoaded && stream && (
        ...
        {!stream && (
          <div className="absolute inset-0 flex items-center justify-center bg-gray-800 z-10">
            <div className="flex flex-col items-center text-white">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-gray-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black to-transparent bg-opacity-70 text-white px-3 py-2 flex items-center">
          {isLocal && (
            <span className="h-2 w-2 bg-green-500 rounded-full mr-2 animate-pulse" aria-label="You"></span>
          )}
        ...
        {/* Emoji reaction */}
        {showEmoji && (
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-5xl animate-bounce z-20 filter drop-shadow-lg" aria-label={`Emoji reaction: ${emoji}`}>
            {emoji}
          </div>
        )}
      </div>
